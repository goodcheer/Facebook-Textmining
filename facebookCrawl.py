# -*- coding: cp949 -*-
#written according to http://dizwe.tistory.com/8
# this version is without comments (only content)
import json
import csv
import datetime
import time
...
print("until: ")
until = input("until: YYY-MM-DD")

# advaced information
# decides what info to get
def getFacebookPageFeedData(page_id, access_token, since, unitl):
    ...
    # parameters2 = "comments.summary(true).filter(stream){message}"  # including comments
    time = "&since=%s&until=%s" % (since, until)
    access = "&access_token=%s" % access_token
    url = base + node + parameters1+ time + access # add parameters2 if needed
    print(url)  ###DEL
